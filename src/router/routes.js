 ...
 * @Last Modified time: 2018-12-14 11:14:57
 */
 ...
 * @property {bool} exact true-精确匹配, 默认false
 * @property {bool} strict 默认false
 * @property {bool} sensitive true-大小写敏感， 默认false
 * @property {string} name 菜单名称, 为空时不在菜单中显示
 * @property {string} icon 菜单图标type，可为空， 目前仅支持antd中Icon
 * @property {bool} hideInMenu 是否在菜单中隐藏， 为空或false时不隐藏
 * @property {string} redirect 重定向路径，可为空
 * @property {string[] | string} authority 路由权限，当用户权限与用户权限不符时菜单中不显示，可选值为： admin-管理员 kf-客服 user-用户， 为空时默认所有权限都可以访问
 * @property {Function Components | Class Components} component 加载组件
 *        异步加载时采用如下写法： lazy(() => import(\/* webpackChunkName: 'test' *\/'../pages/Test'))
 * @property {object[]} routes 子路由，可为空，配置同上
 */
const routes = [
  {
    path: '/login',
